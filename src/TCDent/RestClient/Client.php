...
class Client implements \Iterator, \ArrayAccess
{
    public $options;
    ...
    // Populated as-needed.
    protected $decoded_response; // Decoded response body.
    ...
    {
        throw new ClientException("Decoded response data is immutable.");
    }
    ...
    // Request methods:
    public function get(string $url, $parameters = [], array $headers = []): Client
    {
        return $this->execute($url, 'GET', $parameters, $headers);
    }

    public function post(string $url, $parameters = [], array $headers = []): Client
    {
        return $this->execute($url, 'POST', $parameters, $headers);
    }

    public function put(string $url, $parameters = [], array $headers = []): Client
    {
        return $this->execute($url, 'PUT', $parameters, $headers);
    }

    public function patch(string $url, $parameters = [], array $headers = []): Client
    {
        return $this->execute($url, 'PATCH', $parameters, $headers);
    }

    public function delete(string $url, $parameters = [], array $headers = []): Client
    {
        return $this->execute($url, 'DELETE', $parameters, $headers);
    }

    public function head(string $url, $parameters = [], array $headers = []): Client
    {
        return $this->execute($url, 'HEAD', $parameters, $headers);
    }

    public function execute(string $url, string $method = 'GET', $parameters = [], array $headers = []): Client
    {
        $client = clone $this;
        $client->url = $url;
    ...
    public function decode_response()
    {
        if(empty($this->decoded_response)) {
            ...
            if(!array_key_exists($format, $this->options['decoders'])) {
                throw new ClientException("'${format}' is not a supported ".
